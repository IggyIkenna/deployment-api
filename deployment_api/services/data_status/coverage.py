...
class CoverageStatusMixin(VenueResolutionMixin):
    ...
    async def get_coverage_summary(
        ...
        """Return shard counts and latest-day instrument totals per asset_group (mirrors
        :meth:`get_manifest_status`). Three paths: (1) rollup fast-path — a fresh
        ``coverage.json.gz`` blob served + filtered in-memory, sub-second; (2) live-build OOM
        guard (root-caused 2026-07-26, ``deployment_api_honest_coverage_regression_2026_07_26.md``)
        for known-large services — a pre-flight estimate refuses the build outright (falling back
        to a stale rollup or a structured refusal) before it can OOM the shared container, and even
        an under-budget estimate runs inside a resource-bounded child as defense-in-depth; (3)
        on-demand fall-through — the unguarded synchronous compute. See plan:
        ...
        return await self._coverage_summary_guarded_build(service, asset_groups, cloud)

    async def _coverage_summary_guarded_build(
        self,
        service: str,
        asset_groups: list[str] | None,
        cloud: str,
    ) -> dict[str, object]:
        """Layer 1 (pre-flight refusal) + Layer 2 (resource-bounded child) for OOM-risk services."""
    ...
    async def _coverage_summary_fallback(
        ...
        *,
        ...
        """Serve a stale coverage rollup if one exists, else a structured refusal. Called when the
        pre-flight guard refuses the coverage-summary live build outright, or the bounded child
        failed anyway. Mirrors ``ManifestStatusMixin._live_build_fallback``.
        """
        stale = await asyncio.to_thread(read_coverage_rollup_allow_stale, service)
        if stale is not None:
            return self._stale_coverage_response(service, stale, asset_groups, child_error)
        return self._refused_coverage_response(service, estimate_bytes, child_error)

    @staticmethod
    def _stale_coverage_response(
        service: str,
        stale: tuple[dict[str, object], str | None],
        asset_groups: list[str] | None,
    ...
    @staticmethod
    def _refused_coverage_response(service: str, estimate_bytes: int, child_error: str | None) -> dict[str, object]:
    ...
    def _build_breakdowns(
        ...
        """Build ``breakdowns: dict[axis, dict[value, count]]`` for the deployment-ui
        ``BreakdownsAccordion`` (one section per axis; UAC SSOT shard+display minus primary,
        SHARD-then-DISPLAY order). Empty axes still emit ``{}`` (UI shape leads, backend writers
        follow — the accordion shows a "no data yet" placeholder rather than hiding them).
        ``__legacy__`` surfaces older rows with an empty value (e.g. pre-Phase-1B writes with no
        ``job_id`` stamped yet).
        """
        cat_lower = cat.lower()
    ...
    def _build_coverage_for_cat(self, service: str, cat: str, cloud: str = "gcp") -> dict[str, object] | None:
        ...
        capture_status_counts = self._compute_capture_status_counts(index, shards)
        completion_pct = CoverageStatusMixin._completion_pct(capture_status_counts)
        date_index = self._filter_to_iso_dates(index)
        return self._assemble_coverage_entry(
            service, cat, index, date_index, shards, capture_status_counts, completion_pct
        )

    @staticmethod
    def _completion_pct(capture_status_counts: dict[str, int]) -> float:
        """captured / (captured+empty+failed+expected_unattempted) — denominator EXCLUDES
        out_of_window, since those cells were never collectable."""
        cov_total = (
        ...
        return round(capture_status_counts["captured"] / cov_total * 100, 2) if cov_total > 0 else 0.0

    @staticmethod
    def _compute_capture_status_counts(index: pd.DataFrame, shards: int) -> dict[str, int]:
        """4-state capture_status tally, out-of-window (OOW) partitioned out of the denominator.

        B1: honest breakdown so completion% is captured / (captured+empty+failed+expected_unattempted),
        NOT the self-referential captured/len(index)≈100%. v4 rows without a ``capture_status``
        column are the legacy "every row captured" path.

        OOW partition: within ``empty_confirmed`` rows, those carrying a lifecycle reason
        (pre-genesis chain, pre-launch venue, delisted instrument, post/pre-season, deprecated
        data_type, etc.) are NEVER-COLLECTABLE — they are NOT gaps, so they're split into a
        separate ``out_of_window`` bucket and excluded from the denominator.
        """
        if "capture_status" not in index.columns:
            return {
                "captured": shards,
                "empty_confirmed": 0,
                "attempted_failed": 0,
        ...
        # Legacy/NaN coercion (SSOT parity with ``coverage_metrics.compute_capture_status_counts`` +
        # UTL ``ManifestWriter.lookup``): a blank / NaN / ``"None"`` / ``"nan"`` / any non-4-state
        # capture_status is a legacy pre-v5 row that means "captured". WITHOUT this coercion
        # ``(cs == "captured")`` silently DROPS those rows from BOTH numerator and denominator,
        # under-reporting coverage and diverging from the per-venue breakdown which DOES coerce
        # (sports prd: 17,288 legacy v4 "None" rows dropped, 2026-06-18 audit).
        ...
        oow_count = CoverageStatusMixin._out_of_window_empty_count(index, cs, total_empty)
    ...
    @staticmethod
    def _out_of_window_empty_count(index: pd.DataFrame, cs: pd.Series, total_empty: int) -> int:
        """Count of ``empty_confirmed`` rows that are out-of-coverage-window (never-collectable)."""
        ...
        if total_empty == 0 or _reason_col is None:
            return 0
        empty_mask = cs == "empty_confirmed"
        reasons = index.loc[empty_mask, _reason_col].fillna("").astype(str).str.strip()
        # Vectorised set-membership: ~6x faster than a per-row ``.apply(is_out_of_coverage_window)``
        # over the ~1.2M empty rows a large asset_group carries — same verdict (the frozenset IS
        # the canonical set the helper checks). Blank/calendar reasons stay False.
        oow_reason_mask = reasons.isin(OUT_OF_COVERAGE_WINDOW_REASONS)  # pyright: ignore[reportUnknownMemberType]
        # DATA-TYPE-AWARE schedule-empty resolution (operator direction 2026-06-23): a
        # schedule-DEFINING data_type (sports FIXTURES) that is empty_confirmed with
        # SOURCE_RETURNED_ZERO means "no matches that day = complete" → RESOLVED, out-of-window.
        # An ENRICHMENT data_type's SOURCE_RETURNED_ZERO is NOT excluded (its zero may be a real gap).
        ...
        return int(oow_reason_mask.sum())  # pyright: ignore[reportUnknownMemberType]

    def _assemble_coverage_entry(
        self,
        service: str,
        cat: str,
        index: pd.DataFrame,
        ...
        """Build the final per-cat coverage dict from the precomputed capture-status counts."""
        ...
        # Multi-axis breakdowns for the UI BreakdownsAccordion (empty/absent columns -> {}).
        breakdowns = self._build_breakdowns(service, cat, date_index, group_axis)
        # Deduplicated identity count from the lifecycle catalogue (manifest only carries
        # per-shard counts); None when the AG has no catalogue (e.g. the SHARED pseudo-key).
    ...
    def _get_coverage_summary_sync(
        ...
        # B1: aggregate the per-cat 4-state so the service-level totals carry an honest
        # completion%; out_of_window is tracked separately and EXCLUDED from the denominator.
        total_capture_status: dict[str, int] = dict.fromkeys(
            ("captured", "empty_confirmed", "attempted_failed", "expected_unattempted", "out_of_window"), 0
        )
        ...
        for cat in cat_list:
            ...
            self._fold_cat_entry_into_totals(
                entry, cat, result_categories, all_dates, total_capture_status, running_totals
            )

        completion_pct = CoverageStatusMixin._completion_pct(total_capture_status)
        return {
            "service": service,
            "asset_groups": result_categories,
            "totals": {
                "shards": running_totals["shards"],
                "instrument_rows": running_totals["instrument_rows"],
                "dates_across_categories": len(all_dates),
                "latest_day_instruments": running_totals["latest_day_instruments"],
                "unique_instruments": running_totals["unique_instruments"],
    ...
    @staticmethod
    def _fold_cat_entry_into_totals(
        ...
        """Merge one asset_group's coverage entry into the running service-level totals, in place."""
        ...
        running_totals["shards"] += shards_int
        running_totals["instrument_rows"] += shards_int
        running_totals["latest_day_instruments"] += ld_total_int
        uniq = entry.get("unique_instruments")
        if isinstance(uniq, int):
            running_totals["unique_instruments"] += uniq

...
class ExperimentActionResponse(BaseModel):
    """Response from POST /api/monitor/experiments/{deployment_id}/{action}."""
...
def _gce_action_cmd(vm_name: str, action: str, project_id: str, zone: str) -> str:
    """Build a gcloud compute instances command for stop or restart."""
    if action == "stop":
        return shlex.join(
...
def _run_gce_cmd(cmd_str: str) -> tuple[bool, str]:
    try:
        ...
        return False, "gcloud CLI not found; run from an authenticated terminal"
...
def _do_experiment_action(deployment_id: str, action: str, dry_run: bool) -> ExperimentActionResponse:
    """Look up the experiment VM and issue stop/restart."""
    try:
    ...
    if not _is_experiment_vm(entry.vm_name):
        raise HTTPException(status_code=422, detail=f"{entry.vm_name!r} is not an experiment VM")

    project_id = _cfg.gcp_project_id
    zone = _DEFAULT_GCE_ZONE
    cmd = _gce_action_cmd(entry.vm_name, action, project_id, zone)

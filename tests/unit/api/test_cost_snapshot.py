...
def test_records_roundtrip_preserves_values_and_labels(tmp_path: object, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    out = store.records_in_window("2026-07-01", "2026-08-01", "2026-07-13")
    assert len(out) == 2
    ...
    assert vm.labels == {"purpose": "backfill", "venue": "BINANCE"}
    # empty-labels record round-trips to an empty dict, not "" or None
    assert by_id["b"].labels == {}


def test_records_in_window_recomputes_provisional_and_slices_by_day(
    ...
    out = store.records_in_window("2026-07-09", "2026-07-14", provisional_cutoff_iso="2026-07-11")
    days = sorted(r.day for r in out)
    assert days == ["2026-07-10", "2026-07-12"]
    prov = {r.day: r.is_provisional for r in out}
    assert prov == {"2026-07-10": False, "2026-07-12": True}  # >= cutoff 07-11 → provisional
...
def test_query_empty_when_no_snapshot(tmp_path: object, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert store.records_in_window("2026-07-01", "2026-08-01", "2026-07-13") == []
...
# --- _load_window: snapshot preferred, live fallback -------------------------
class _FakeStore:
    ...
    def records_in_window(self, _s: str, _e: str, _c: str) -> list[CostRecord]:
        return self._recs
...
def test_load_window_falls_back_when_snapshot_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    out = s._load_window(date(2026, 7, 1), date(2026, 8, 1))  # pyright: ignore[reportPrivateUsage]
    assert [r.resource_id for r in out] == ["LIVE"]  # snapshot error degrades to live, never 5xx

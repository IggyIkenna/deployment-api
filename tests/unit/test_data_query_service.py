...
# Load DataQueryService directly to avoid __init__ import issues
import importlib
import sys
import types
...
def _load_dqs():
    """Load DataQueryService module with mocked settings."""
    # Patch settings and storage_facade before import
    mock_settings = types.ModuleType("deployment_api.settings")
    mock_settings.GCP_PROJECT_ID = "test-project"  # type: ignore[attr-defined]

    with patch.dict(sys.modules, {"deployment_api.settings": mock_settings}):
...
class TestListFilesInPath:
    ...
    async def test_empty_bucket_returns_empty_files(self):
        ...
        with patch("deployment_api.services.data_query_service.list_objects", return_value=[]):
    ...
    async def test_files_returned(self):
        ...
        with patch("deployment_api.services.data_query_service.list_objects", return_value=[blob]):
    ...
    async def test_truncated_when_exceeds_max(self):
        ...
        with patch("deployment_api.services.data_query_service.list_objects", return_value=blobs):
    ...
    async def test_error_returns_error_dict(self):
        svc = self._make_service()
        with patch("deployment_api.services.data_query_service.list_objects", side_effect=OSError("bucket not found")):
...
class TestGetVenueFilters:
    ...
    async def test_known_service_returns_venues(self):
        svc = self._make_service()
        with patch("deployment_api.services.data_query_service.list_prefixes", return_value=["BINANCE/", "OKX/"]):
    ...
    async def test_exception_handled_per_category(self):
        svc = self._make_service()
        with patch(
            "deployment_api.services.data_query_service.list_prefixes",
            side_effect=OSError("bucket missing"),
        ):
            result = await svc.get_venue_filters("instruments-service")
...
class TestGetInstrumentsList:
    ...
    async def test_venue_filter_builds_path(self):
        svc = self._make_service()
        with patch("deployment_api.services.data_query_service.list_objects", return_value=[]) as mock_list:
    ...
    async def test_error_returns_error_dict(self):
        svc = self._make_service()
        with patch("deployment_api.services.data_query_service.list_objects", side_effect=RuntimeError("error")):
            result = await svc.get_instruments_list("cefi")
        assert "error" in result
...
class TestGetInstrumentAvailability:
    ...
    async def test_cefi_venue_returns_cefi_category(self):
        svc = self._make_service()
        with patch("deployment_api.services.data_query_service.object_exists", return_value=False):
    ...
    async def test_tradfi_venue_category_detection(self):
        svc = self._make_service()
        with patch("deployment_api.services.data_query_service.object_exists", return_value=True):
            result = await svc.get_instrument_availability(
    ...
    async def test_data_type_passed_used(self):
        ...
        with patch("deployment_api.services.data_query_service.object_exists", return_value=False):
            result = await svc.get_instrument_availability(
                "BINANCE", "SPOT", "BTC-USDT", "2026-01-01", "2026-01-01",
